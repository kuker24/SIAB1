...
RUNTIME="$ROOT/runtime_control/nginx.start-canary.conf"
OFF="$ROOT/docker/nginx.start-canary-off.conf"
...
mode_file() {
  ...
  printf "%s\n" "$ROOT/docker/nginx.start-canary-${name}.conf"
}
...
apply_mode() {
  ...
  # Overwrite the existing inode so the nginx bind-mount sees the change.
  cat "$src" > "$RUNTIME"
  reload_nginx
  printf "canary_mode=%s\n" "$(current_mode)"
}
