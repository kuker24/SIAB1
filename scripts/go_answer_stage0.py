...
PREFIX = os.getenv("GOANS_PREFIX", "GOANS0")
CLASS_NAME = "XII-GO-ANS"
PHASES = (10, 25, 50)
SEB_KEY = "ans-stage0-seb"
...
def answer_one(base: str, token: str, session_id: int, question_id: int, option_id: int) -> dict[str, Any]:
    started = datetime.now(timezone.utc)
    response = httpx.post(
        f"{base.rstrip('/')}/api/exams/submit-answer",
        headers={
        ...
        json={"session_id": session_id, "question_id": question_id, "selected_option_id": option_id},
        timeout=30.0,
    )
    body: Any
    try:
...
async def seed(conn: asyncpg.Connection, count: int) -> dict[str, Any]:
    ...
    teacher = int(
        await conn.fetchval(
            ...
            VALUES ($1, 'x', 'Go Ans0 Teacher', 'teacher', true) RETURNING id
            """,
            f"{PREFIX}_teacher",
        )
    )
    exam_id = int(
        await conn.fetchval(
            ...
            INSERT INTO exams (
                ...
                $1, $2, 90, $3, $4, 3, false, false, false, $5,
                true, 'MTK', 'UTS', true, 'ANS000', false, false
            ) RETURNING id
            """,
            f"{PREFIX}_exam",
            teacher,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage-0 synthetic ANSWER against Go")
    parser.add_argument("--go-url", default=os.getenv("GO_ANSWER_URL", "http://go_server:8000"))
    parser.add_argument("--answer-url", default=os.getenv("GO_ANSWER_HTTP_URL", ""))
...
async def amain() -> dict[str, Any]:
    ...
    answer_url = (args.answer_url or args.go_url).rstrip("/")
    ...
    report: dict[str, Any] = {"phases": [], "errors": [], "answer_url": answer_url}
    try:
        ...
        for count in phases:
            ...
            with ThreadPoolExecutor(max_workers=count) as pool:
                results = list(
                    pool.map(
                        lambda pair: answer_one(
                            answer_url, pair[0], pair[1], fixture["question_id"], fixture["option_id"]
                        ),
            ...
            saved = int(
                await conn.fetchval(
                    "SELECT count(*) FROM answers WHERE session_id = ANY($1::int[])",
                    slice_sessions,
                )
                or 0
            )
