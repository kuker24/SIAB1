...
def test_student_exam_write_handlers_proxy_instead_of_local_mutation() -> None:
    http_src = (EXAM / "http.go").read_text(encoding="utf-8")
    start_src = (EXAM / "start.go").read_text(encoding="utf-8")
    ...
    assert "CreateSession" not in start_src
